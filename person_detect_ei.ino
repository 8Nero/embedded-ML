...
/* Includes ---------------------------------------------------------------- */
#include <Person_Detection2_inferencing.h>
...
const char* serverUrl = "http://192.168.1.180:5001/upload";  // Adjust server address
...
// Function to capture and send image to server
void sendImage() {
  ...
  if (WiFi.status() == WL_CONNECTED) {
    ...
    Serial.println("Initalizing HTTP connection . . .");
    ...
    if (httpResponseCode > 0) {
      ...
      Serial.println("Image sent, response: " + response);
    } 
...
void loop()
{
    ...
    // Send person image to server
    if (result.classification[1].value > 0.8){
          sendImage();  // Send image if captured successfully
        }
